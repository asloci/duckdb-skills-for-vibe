...
name: read-memories
description: >
  Search past Vibe session logs to recall prior decisions, patterns, or unresolved work.
...
Search paths:
- All projects: `$HOME/.vibe/logs/session/*.jsonl`
- Current only (`--here`): `$HOME/.vibe/logs/session/$(echo "$PWD" | sed 's|[/_]|-|g')/*.jsonl`
